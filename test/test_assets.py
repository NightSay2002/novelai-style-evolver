...
class AssetTests(unittest.TestCase):
    def test_artist_pool_supports_the_configurable_50_post_minimum(self):
        path = pathlib.Path(__file__).resolve().parents[1] / "data" / "artists.json"
        data = json.loads(path.read_text(encoding="utf-8"))
        self.assertEqual(data["minimumPostCountExclusive"], 50)
        self.assertEqual(len(data["artists"]), 52266)
        self.assertTrue(all(item["postCount"] >= 50 for item in data["artists"]))
